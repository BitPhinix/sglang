...
class ForwardBatch:
    ...
    def init_new(
        ...
        if batch.global_num_tokens is not None:
            # process global_num_tokens
            if batch.spec_num_draft_tokens is not None:
                global_num_tokens = [
                    x * batch.spec_num_draft_tokens for x in batch.global_num_tokens
                ]
            else:
                global_num_tokens = batch.global_num_tokens

            # process global_num_tokens_for_logprob
            if (
                batch.spec_num_draft_tokens is not None
                and not batch.forward_mode.is_draft_extend()
            ):
                global_num_tokens_for_logprob = [
                    x * batch.spec_num_draft_tokens
